...
class MessagesCommand(val plugin : ChatZ) : CommandExecutor {
    ...
    companion object {
        @JvmField val MESSAGE_EVENTS = arrayOf("join", "quit", "death-generic", "death-pvp")
    }
    ...
    override fun onCommand(sender: CommandSender, command: Command, label: String, args: Array<out String>): Boolean {
                ...
                if (args.size == 1) {
                    ...
                    if (sender !is Player ) {
                        ...
                        val clickHere : Array<BaseComponent> = ComponentBuilder("Click Here").color(ChatColor.DARK_GREEN)
                                .event(ClickEvent(ClickEvent.Action.SUGGEST_COMMAND, "/messages $message ${format?:""}"))
        ...
        val cmd = "/$label "
        sender.sendMessage(Messages.SEPARATOR)
        MESSAGE_EVENTS.forEach { sender.sendMessage(cmd + it + Messages.GRAY + " Change the " + it + " message format") }
        sender.sendMessage("")
        if (sender !is Player) {
            sender.sendMessage(Messages.GREEN + "Type " + Messages.DARK_GREEN + "/placeholders" + Messages.GREEN + " to view the available placeholders.")
        } else {
